...
/**
 * 可定制图标的水波纹进度球 && 包含进度球color定制与icon定制
 */
public class YWaveLoadView extends View {
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            mIsShowText = ta.getBoolean(R.styleable.WaveLoadView_wave_load_show_text, false);
    ...
    private void initAnim() {
        ...
        final ValueAnimator mOffsetAnimator1 = ValueAnimator.ofInt(0, mItemWidth);
        ...
        mOffsetAnimator1.setDuration(900);
        mOffsetAnimator1.setRepeatCount(-1);

        final ValueAnimator mOffsetAnimator2 = ValueAnimator.ofInt(0, mItemWidth / 2);
        ...
        mOffsetAnimator2.setDuration(1500);
        ...
        mProgressAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                //指定进度位置，超过该位置后就不在继续向上加载动画
                if (mWaterTop < (getMeasuredHeight() - (mIndex * getMeasuredHeight()))) {
                    mWaterTop = (int) (getMeasuredHeight() - (mIndex * getMeasuredHeight()));
                }
        ...
        mWaveHeightAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                //控制当水波纹到达指定区间位置后，停留在重点位置一直重复执行水波纹动画
                if (mWaterTop >= (getMeasuredHeight() - (mIndex * getMeasuredHeight()))) {
                    //控制到达指定区间时，水波幅度
                    mWaveHeight = (int) (getMeasuredHeight() - (mIndex * getMeasuredHeight())) / 12;
                } else {
     ...
     * @param count 总长
     */
    public void setProgress(float index, float count) {
        mIndex = index / count;
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        /*关闭初始化控件时就直接加载动画 , mIsAutoLoad控制是否打开自动*/
        if (mIsAutoLoad) {
            startLoad();
        }
    }
